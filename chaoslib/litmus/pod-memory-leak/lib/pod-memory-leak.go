...
// injectChaosInSerialMode inject the Memory Leak Chaos in all target application serially (one by one)
func injectChaosInSerialMode(experimentsDetails *experimentTypes.ExperimentDetails, targetPodList apiv1.PodList, clients clients.ClientSets, chaosDetails *types.ChaosDetails, resultDetails *types.ResultDetails, eventsDetails *types.EventDetails) error {
	...
	// creating the helper pod to perform Memory Leak Chaos
	for _, pod := range targetPodList.Items {
		...
		//Deleting all the helper pod for pod-memory-leak chaos
...
// injectChaosInParallelMode inject the Memory Leak Chaos in all target application in parallel mode (all at once)
...
func getPodEnv(experimentsDetails *experimentTypes.ExperimentDetails, podName string) []apiv1.EnvVar {
	...
	envDetails.SetEnv("APP_NS", experimentsDetails.AppNS).
		...
		SetEnv("TARGET_MEMORY_CONSUMPTION", strconv.Itoa(experimentsDetails.TargetMemoryConsumption)).
